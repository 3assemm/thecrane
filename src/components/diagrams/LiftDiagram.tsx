...
interface LiftDiagramProps {
  boomAngle: number;
  buildingHeight: number;
  liftRadius: number;
  craneEdgeDistance: number; // Add this prop
}

export const LiftDiagram: React.FC<LiftDiagramProps> = ({
  boomAngle,
  buildingHeight,
  liftRadius,
  craneEdgeDistance // Use this prop
}) => {
  // Calculate boom end point using lift radius and trigonometry
  const boomEndX = liftRadius;
  const boomEndY = Math.tan(boomAngle * Math.PI / 180) * boomEndX;
    ...
    >
      ...
      <div className="h-[600px] w-full mx-auto bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
        <ResponsiveContainer width="100%" height="100%" aspect={1}>
          ...
          >
            ...
            <ReferenceLine
              segment={[
                { x: craneEdgeDistance, y: 0 },
                { x: craneEdgeDistance, y: buildingHeight }
              ]}
              stroke="#4B5563"
              strokeWidth={4}
              label={{
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 text-sm">
        ...
        <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
          ...
          <ul className="space-y-2 text-gray-600 dark:text-gray-400">
            <li>Height: {buildingHeight}m</li>
            <li>Distance: {craneEdgeDistance}m</li>
            <li>Lift Radius: {liftRadius}m</li>
          </ul>
        </div>
      </div>
    </motion.div>
